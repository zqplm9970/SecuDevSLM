...
y_ios = data_ios['iOS Noise Character Category'].values
...
y_android = data_android['android Noise Character Category'].values
...
for j in range(len(outliers_ios)):
    if outliers_ios[j]:
        ...
        if outliers_android[j]:  # If it's an overlap
            ...
            plt.scatter(x_ios[j], y_ios[j], color='purple', alpha=0.7, marker='o')  # black for overlapping points
            plt.text(x_ios[j] - 0.66, y_ios[j] + 0.06, f"a{outlier_count_ios} ",fontsize=13, ha='center', va='bottom', fontweight='bold')
        else:
            plt.scatter(x_ios[j], y_ios[j], color='blue', marker='o')  # Highlight in blue
...
for j in range(len(outliers_android)):
    if outliers_android[j]:
        ...
        if outliers_ios[j]:  # If it's an overlap already handled
            ...
            plt.text(x_ios[j] + 0.66, y_ios[j] + 0.06, f' /b{outlier_count_android}', fontsize=13, ha='center', va='bottom', fontweight='bold')
        else:
...
plt.ylabel('Noise Character Category', fontweight='bold', fontsize=20)
plt.legend()
