...
# Load the CSV file
file_path = './model_result.csv'
data = pd.read_csv(file_path, encoding='latin1')

# Renaming the columns based on the provided description
data.columns = [
    'Model Name', 
    'Performance Rank', 
...
x = data['Performance Rank'].values.reshape(-1, 1)
y = data['iOS Noise Text Length'].values
...
# Calculate the threshold for identifying outliers (e.g., values more than 2 standard deviations from the line)
...
# Highlight the outliers in black and annotate their model names
for j in range(len(outliers)):
    if outliers[j]:
        plt.scatter(x[j], y[j], color='black')
        plt.text(x[j], y[j] + 2, data['Model Name'][j], fontsize=7, ha='center', va='bottom')

# Set labels and title
plt.xlabel('Performance Rank', fontweight='bold',fontsize=20)
plt.ylabel('iOS Noise Text Length', fontweight='bold',fontsize=20)
plt.title('Scatter plot and Fit for iOS Noise Text Length', fontweight='bold',fontsize=20)
plt.legend()
...
y_ios = data_ios['iOS Noise Text Length'].values
...
y_android = data_android['android Noise Text Length'].values
...
plt.scatter(x_ios, y_ios, label='iOS Data', color='blue', marker='o')
plt.plot(x_ios, y_pred_ios, color='blue', label='iOS Fit Line', linestyle='-')
...
for j in range(len(outliers_ios)):
    if outliers_ios[j]:
        ...
        if outliers_android[j]:  # If it's an overlap
            ...
            plt.scatter(x_ios[j], y_ios[j], color='purple', alpha=0.7)  # Purple for overlapping points
            plt.text(x_ios[j] - 0.66, y_ios[j] + 1, f"a{outlier_count_ios} ",fontsize=13, ha='center', va='bottom', fontweight='bold')
        else:
            plt.scatter(x_ios[j], y_ios[j], color='blue')  # Highlight in blue
            plt.text(x_ios[j], y_ios[j] + 1, f'a{outlier_count_ios}', fontsize=13, ha='center', va='bottom', fontweight='bold')
...
for j in range(len(outliers_android)):
    if outliers_android[j]:
        ...
        if outliers_ios[j]:  # If it's an overlap already handled
            # Here, we use the iOS and Android sequence indices directly
            plt.scatter(x_ios[j], y_ios[j], color='purple', alpha=0.7,marker='^')  # Purple for overlapping points
            plt.text(x_ios[j]+ 0.66 , y_ios[j] + 1, f' /b{outlier_count_android}', fontsize=13, ha='center', va='bottom', fontweight='bold')
        else:
            plt.scatter(x_android[j], y_android[j], color='red',marker='^')  # Highlight in red
            plt.text(x_android[j], y_android[j] + 1, f'b{outlier_count_android}', fontsize=13, ha='center', va='bottom', fontweight='bold')
...
plt.ylabel('Noise Text Length', fontweight='bold', fontsize=20)
plt.legend()
