...
file_path_ios = './model_1.csv'
data_ios = pd.read_csv(file_path_ios, encoding='latin1')

file_path_android = './model_1.csv'
...
data_ios.columns = [
    ...
    'Parameter volume',
    'Performance Rank', 
    'Types of iOS jailbreak behaviors', 
    'Types of Android jailbreak behaviors', 
    'Does iOS trigger code jailbreak', 
    'Does Android trigger code jailbreak'
]
...
x_ios = (data_ios['data diversity'] * np.log(1 + data_ios['Data volume'])).values.reshape(-1, 1)
y_ios = data_ios['Types of iOS jailbreak behaviors'].values
...
x_android = (data_android['data diversity'] * np.log(1 + data_android['Data volume'])).values.reshape(-1, 1)
y_android = data_android['Types of Android jailbreak behaviors'].values
...
for j in range(len(outliers_ios)):
    if outliers_ios[j]:
        ...
        if outliers_android[j]:  # If it's an overlap
            ...
            plt.text(x_ios[j]-0.07 , y_ios[j] + 0.06, f"a{outlier_count_ios} ",fontsize=13, ha='center', va='bottom')
        else:
            plt.scatter(x_ios[j], y_ios[j], color='blue')  # Highlight in blue
            plt.text(x_ios[j], y_ios[j] + 0.06, f'a{outlier_count_ios}', fontsize=13, ha='center', va='bottom')
...
for j in range(len(outliers_android)):
    if outliers_android[j]:
        ...
        if outliers_ios[j]:  # If it's an overlap already handled
            ...
            plt.text(x_ios[j] + 0.07, y_ios[j] + 0.06, f' /b{outlier_count_android}', fontsize=13, ha='center', va='bottom')
        else:
            plt.scatter(x_android[j], y_android[j], color='red',marker='^')  # Highlight in red
            plt.text(x_android[j], y_android[j] + 0.06, f'b{outlier_count_android}', fontsize=13, ha='center', va='bottom')
        
        android_outliers_text.append(f'b{outlier_count_android}. {data_android["Model Name"][j]}')
        outlier_count_android += 1

# Set labels and title
plt.xlabel('Data Richness', fontweight='bold', fontsize=20)
plt.ylabel('Number of Behavior Categories', fontweight='bold', fontsize=20)
plt.legend()
...
plt.figtext(0.81, 0.80, f'iOS Outliers\n{ios_text}', fontsize=10, ha='left', va='top', bbox=dict(facecolor='white', alpha=0.8, edgecolor='none'), fontweight='bold')
plt.figtext(0.81, 0.64, f'Android Outliers\n{android_text}', fontsize=10, ha='left', va='top', bbox=dict(facecolor='white', alpha=0.8, edgecolor='none'), fontweight='bold')
