...
y_ios = data_ios['iOS Illusion Attack Rounds'].values
...
y_android = data_android['android Illusion Attack Rounds'].values
...
for j in range(len(outliers_ios)):
    if outliers_ios[j]:
        ...
        if outliers_android[j]:  # If it's an overlap
            ...
            plt.text(x_ios[j] - 0.66, y_ios[j] + 0.1, f"a{outlier_count_ios} ",fontsize=12, ha='center', va='bottom', fontweight='bold')
        else:
            plt.scatter(x_ios[j], y_ios[j], color='blue')  # Highlight in blue
            plt.text(x_ios[j], y_ios[j] + 0.1, f'a{outlier_count_ios}', fontsize=12, ha='center', va='bottom', fontweight='bold')
...
for j in range(len(outliers_android)):
    if outliers_android[j]:
        ...
        if outliers_ios[j]:  # If it's an overlap already handled
            ...
            plt.text(x_ios[j] + 0.66, y_ios[j] + 0.1, f' /b{outlier_count_android}', fontsize=12, ha='center', va='bottom', fontweight='bold')
        else:
            plt.scatter(x_android[j], y_android[j], color='red',marker='^')  # Highlight in red
            plt.text(x_android[j], y_android[j] + 0.1, f'b{outlier_count_android}', fontsize=12, ha='center', va='bottom', fontweight='bold')
...
plt.ylabel('Illusion Attack Rounds', fontweight='bold', fontsize=20)
plt.legend()
