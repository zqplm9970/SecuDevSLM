...
y_label_file1 = "iOS Noise Attack Rounds"  
y_label_file2 = "Android Noise Attack Rounds"  
...
ios_moving_avg, ios_upper_band, ios_lower_band = calculate_bollinger_bands(ios_data["ios噪声攻击轮次"])
android_moving_avg, android_upper_band, android_lower_band = calculate_bollinger_bands(android_data["android噪声攻击轮次"])

ax.plot(ios_data["性能排名"], ios_data["ios噪声攻击轮次"], 
        color='blue', label=f'iOS', marker='o', markersize=8, 
        linewidth=2, alpha=0.9, markerfacecolor='blue', markeredgecolor='blue')

ax.plot(android_data["性能排名"], android_data["android噪声攻击轮次"], 
...
ax.set_ylabel("Noise Attack Rounds", fontsize=30, fontweight='bold')
...
y_min = min(ios_data["ios噪声攻击轮次"].min(), android_data["android噪声攻击轮次"].min())
y_max = max(ios_data["ios噪声攻击轮次"].max(), android_data["android噪声攻击轮次"].max())
